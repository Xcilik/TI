...
*_Powered by Ti Unusia Bot._*      
 ...
 // Pastikan daftarAcara dibuat global
            ...
            case 'play': {
                if (!args[0]) return replygcxeon(`Masukkan judul lagu!\nContoh: *.play senja bersemi*`)
                
                try {
                    let query = args.join(" ")
                    let res = await fetch(`https://api.nekorinn.my.id/downloader/ytplay-savetube?q=${encodeURIComponent(query)}`)
...
// Kirim gambar dengan caption saja
                    ...
                    console.error(e)
                    replygcxeon('Terjadi kesalahan saat mengambil data.')
                }
            }
            break
            case 'ytmp3': {
